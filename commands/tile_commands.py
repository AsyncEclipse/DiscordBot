...
class TileCommands(commands.GroupCog, name="tile"):
    ...
    @app_commands.command(name="manage_population",
                          description="add or remove population cubes (using positive or negative numbers) from a tile")
    @app_commands.choices(color=color_choices)
        ...
        if added_pop:
            ...
            for x in range(neutralPop):
                ...
                view.add_item(Button(label="Material", style=discord.ButtonStyle.gray,
                                     custom_id=f"FCID{player_color}_reducePopFor_material"))
                view.add_item(Button(label="Science", style=discord.ButtonStyle.gray,
                                     custom_id=f"FCID{player_color}_reducePopFor_science"))
                view.add_item(Button(label="Money", style=discord.ButtonStyle.gray,
                                     custom_id=f"FCID{player_color}_reducePopFor_money"))
                await interaction.channel.send(f"{playerObj['player_name']}, "
                                               "choose what type of cube to put on the neutral planet", view=view)
            for x in range(orbitalPop):
                ...
                view.add_item(Button(label="Money", style=discord.ButtonStyle.gray,
                                     custom_id=f"FCID{player_color}_reducePopFor_money"))
                await interaction.channel.send(f"{playerObj['player_name']}, "
                                               "choose what type of cube to put on the orbital", view=view)
        if removed_pop:
    ...
    @app_commands.choices(color=color_choices)
    async def remove_influence(self, interaction: discord.Interaction, tile_position: str,
    ...
    async def explore_specific_system_tile(self, interaction: discord.Interaction, tile_position: str, system_num: str):
        ...
        await interaction.followup.send(file=drawing.show_single_tile(image))
        view = View()
        button = Button(label="Place Tile", style=discord.ButtonStyle.green,
                        custom_id=f"placeTile_{tile_position}_{tile}_{orientation}")
    ...
    async def resolve_specific_discovery_tile(self, interaction: discord.Interaction,
                                              tile_position: str, discovery_id: str):
        game = GamestateHelper(interaction.channel)
        disc = discovery_id
        tile = tile_position
        await interaction.response.defer(thinking=False)
        ...
        view.add_item(Button(label="Use it for its ability", style=discord.ButtonStyle.green,
                             custom_id=f"usedDiscForAbility_{disc}_{tile}"))
