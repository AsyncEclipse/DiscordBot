...
class PulsarButtons:
    ...
    def findPulsarOptions(game: GamestateHelper, player):
        ...
        for tile in game.gamestate["board"]:
            if "currentAction" in game.gamestate["board"][tile] and game.gamestate["board"][tile]["owner"] == player["color"] and ("activatedPulsars" not in player or tile not in player["activatedPulsars"]):
                for action in actions:
                    if action == game.gamestate["board"][tile]["currentAction"]:
                        continue
                    view.add_item(Button(label=f"Do Pulsar "+action.capitalize()+" ("+tile+")", style=discord.ButtonStyle.gray, custom_id=f"FCID{player['color']}_pulsarAction_{tile}_{action}"))
        return view
    @staticmethod
    async def pulsarAction(game: GamestateHelper, player, interaction:discord.Interaction, player_helper:PlayerHelper, customID:str):
        ...
        await interaction.channel.send(player["player_name"]+ " used a pulsar (tile "+tile+") to perform the "+action+" action")
        if action == "build":
