...
class ExploreButtons:
    ...
    def getListOfTilesPlayerIsIn(game, player):
        ...
        for tile in tile_map:
            if ("owner" in tile_map[tile] and tile_map[tile]["owner"] == player["color"]) or ("player_ships" in tile_map[tile] and ExploreButtons.doesPlayerHaveUnpinnedShips(player,tile_map[tile]["player_ships"],game, tile)):
                tiles.append(tile)
        return tiles
    @staticmethod
            ...
            if "mon" not in ship:
                if player["color"] in ship and "orb" not in ship:
                    ...
                    if "orb" in ship and ship.split("-")[0] is not player["color"]:
                        ...
                        if game.find_player_faction_name_from_color(color) == "The Exiles" and "orbital_pop" in game.gamestate["board"][tile] and game.gamestate["board"][tile]["orbital_pop"][0]==1:
                            opponentShips += 1
    ...
    async def exploreTile(game: GamestateHelper, player, interaction: discord.Interaction, customID:str):
        ...
        if len(msg) > 2:
            ...
            if player["name"]=="Descendants of Draco":
                ...
                if ring != 3 or discard + len(game.gamestate[f"tile_deck_300"]) > 0:
                    ...
                    await interaction.channel.send("Option #1",file=await asyncio.to_thread(drawing.show_single_tile,image))
                    await interaction.channel.send("Option #2",file=await asyncio.to_thread(drawing.show_single_tile,image2))
        ...
        await interaction.channel.send("Tile explored in position "+position+": Tile "+tileID,file=file2)
        playerTiles = ExploreButtons.getListOfTilesPlayerIsIn(game, player)
        view, file = await asyncio.to_thread(drawing.draw_possible_oritentations,tileID,position,playerTiles, view,player)
