...
class DiplomaticRelationsButtons:
    ...
    async def startMinorRelations(game: GamestateHelper, player, interaction: discord.Interaction):
        ...
        await interaction.channel.send(f"{player['player_name']}, "
                                       "choose which minor species you would like to recruit.",
                                       view=view, file=await asyncio.to_thread(drawing.show_minor_species))

    @staticmethod
    async def formMinorRelations(game: GamestateHelper, player, interaction: discord.Interaction,
                                 buttonID: str, player_helper: PlayerHelper):
        await interaction.message.delete()
        minor = buttonID.split("_")[1]
        game.formMinorSpeciesRelations(player, minor)
        await interaction.channel.send(f"{player['player_name']} formed relations with the minor species"
                                       f" that gives the benefit of {minor}.")
        cost = DiplomaticRelationsButtons.getMinorSpeciesCost(minor)
